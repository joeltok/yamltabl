...
import { IJsonInput, IJsonInputMetadata, IJsonInputRows } from './types.js';
import { ValidationError } from './errors.js';

export function validateJsonInput({ metadata, rows }: IJsonInput): void {
...
function validateYamltableField(yamlField: string): void {
  if (yamlField !== '1.0.0') {
    throw new Error('missing field "yamltable"');
  }
}
