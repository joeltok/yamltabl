  ...
  it('simple example', () => {
    const jsonTableRemapped = {
      rows: {
        ...
        row2: {
          column1: ['bullet point', 'bullet point'],
        },
      },
    };
    ...
    expect(minify(result.rows['row2']['column1'] as string)).toEqual(
      minify(expected.rows['row2']['column1'])
    );
  });
});
...
describe('reformatCell', () => {
  ...
  it('solo-leveling bullet points: converted to html', () => {
    const cell = ['bullet point', 'bullet point'];
    const expected = `
      <ul>
        <li>bullet point</li>
        <li>bullet point</li>
  ...
  it('double-level bullet points: converted to html', () => {
    const cell = [
      {
        'parent bullet point': ['child bullet point', 'child bullet point'],
      },
      {
        'parent bullet point': ['child bullet point', 'child bullet point'],
      },
    ...
    const cell = [
      {
        'parent bullet point': [
          {
            'child bullet point': ['baby bullet point', 'baby bullet point'],
          },
          {
            'child bullet point': ['baby bullet point', 'baby bullet point'],
          },
